...
public class EndlessTerrain : MonoBehaviour
{
	...
	private void FixedUpdate() 
	{
		...
		if (timeSinceLastSim >= timeToSim)
		{
			...
			{
				if (chunk.cellGrid.hasIgnitedCell)
				{
					chunk.cellGrid.Simulate();

					for (int i = 0, x = 0; x < chunk.cellGrid.grid.Count; x++)
					{
						for (int y = 0; y < chunk.cellGrid.grid[0].Count; y++)
						{
							switch(chunk.cellGrid.grid[x][y].state)
							{
								/* case CellState.burnable:
									chunk.mapData.colourMap[i] = new Color(79f/255, 119f/255, 45f/255);
									break;
								
								case CellState.not_burnable:
									chunk.mapData.colourMap[i] = new Color(0f/255, 150f/255, 199f/255);
									break; */

								case CellState.ignited:
									chunk.mapData.colourMap[i] = new Color(220f/255, 47f/255, 2f/255);
									break;

								case CellState.on_fire:
									chunk.mapData.colourMap[i] = new Color(208f/255, 0f/255, 0f/255);
									break;
								
								case CellState.extenguishing:
									chunk.mapData.colourMap[i] = new Color(255f/255, 186f/255, 8f/255);
									break;

								case CellState.fully_extenguished:
									chunk.mapData.colourMap[i] = new Color(22f/255, 26f/255, 29f/255);
									break;
							}
	...
	public class TerrainChunk
	{
		...
		void OnMapDataReceived(MapData mapData)
		{
			...
			this.cellGrid = new CellGrid(mapData.vegetationMap, mapData.elevationMap);
